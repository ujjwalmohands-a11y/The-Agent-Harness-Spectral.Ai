...
export function RuixenGradientFooter({
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      // How much scroll is left before the end of the page.
      ...
      // Don't interrupt if we are already smoothly snapping back
      if (isSnapping) {
        if (t <= 0) isSnapping = false;
        return;
      }
      ...
      scrollTimeout = setTimeout(() => {
        // If they stopped scrolling and the gradient is visible (t > 0)
        if (t > 0) {
          isSnapping = true;
          // Calculate the exact scroll position where the gradient is fully hidden
          const snapPosition = doc.documentElement.scrollHeight - win.innerHeight - h;
          win.scrollTo({ top: snapPosition, behavior: 'smooth' });

          // Failsafe: reset snapping flag after animation completes
  ...
  return (
    // Re-added paddingBottom so there is empty space to scroll into.
    <footer
      className={className}
